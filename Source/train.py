...
def train_gan(folder_list, model_name="", epoch_number=10, batch_size=8, num_workers=4):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if model_name == "":
        model_name = "ymodel_" + "GAN" + "_epochnumber" + str(epoch_number)
    save_directory = join(models_directory, model_name)
    Path(save_directory).mkdir(parents=True, exist_ok=True)
    temp_directory = join(save_directory, "temp")
    Path(temp_directory).mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(epoch_number):
        ...
        for i, (previous_tensor, following_tensor, target_tensor) in enumerate(train_loader):
            ...
            discrimination = discriminator(interpolation_tensor)

            alpha = exp(-gamma*epoch)
            loss = alpha * criterion(interpolation_tensor, target_tensor) + adversarial_loss_criterion(discrimination, torch.ones_like(discrimination))
            optimiser.zero_grad()
            loss.backward()
